...
- [x] 1.1 Create `skills/concord/SKILL.md` with frontmatter: kebab-case `name: concord`, a
...
- [x] 1.2 Write the "when to run" section: before archiving a change, after rebasing/merging the
  base branch into a change branch, and when opening/editing a change targeting an existing
  requirement.
- [x] 1.3 Write the "how to invoke" section: `concord check` and `concord overlap`, the `--json`,
...
- [x] 1.4 Write the "interpret & resolve" section: the exit-code contract (0/1/2) and one entry
...
- [x] 1.5 Use British English in prose to match repo convention.

## 2. Add Claude plugin metadata and directories

- [x] 2.1 Create `.claude-plugin/plugin.json` with `name: concord` (kebab-case) and
...
- [x] 2.2 Create `.claude-plugin/marketplace.json` listing the concord plugin with a relative
  `"./"` source, an `owner` (name), a marketplace `name`, and a listed plugin `name` matching the
  manifest.
- [x] 2.3 Confirm the root `skills/` tree (from task 1) is the layout the manifest expects and
  that no path in `plugin.json` contradicts it.

## 3. Documentation

- [x] 3.1 Add an install/usage section to `README.md`: `/plugin marketplace add lucinate-ai/concord`
...
- [x] 3.2 Add a note to `AGENTS.md`'s Release section to bump `plugin.json`'s `version` alongside
  `package.json`, and to update the skill when concord's finding kinds or flags change.

## 4. Validate

- [x] 4.1 Verify `.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json` are valid
...
- [x] 4.2 Cross-check the skill against the specs: every listed trigger, flag, exit code, and
  finding kind is covered, and the description mentions concord/OpenSpec/drift/overlap.
- [x] 4.3 Run `pnpm lint` / `pnpm test` to confirm the additive files don't disturb the existing
