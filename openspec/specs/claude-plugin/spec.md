...
Make the concord repository installable as a Claude Code plugin, so the concord agent skill (and concord's existing slash commands) reach users in one step. The repo carries the plugin manifest, the auto-discovered skill layout, and a self-hosting marketplace catalogue — no separate registry repo. This is packaging and distribution only; it adds no runtime behaviour and changes none of concord's commands, `--json` shapes, or exit codes.
